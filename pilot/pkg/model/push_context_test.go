...
func TestMergeGatewaysUsesSelectorBeforeServiceAnnotation(t *testing.T) {
	const namespace = "gateways"
	makeGateway := func(name string) config.Config {
		return config.Config{
			Meta: config.Meta{
				Name:      name,
				Namespace: namespace,
				Annotations: map[string]string{
					InternalGatewayServiceAnnotation: name + ".gateways.svc.cluster.local",
				},
			},
			Spec: &networking.Gateway{
				Selector: map[string]string{"gateway.networking.k8s.io/gateway-name": name},
	...
	allNamespaces := makeGateway("all-namespaces")
	sameNamespace := makeGateway("same-namespace")
	push := &PushContext{gatewayIndex: gatewayIndex{all: []config.Config{sameNamespace, allNamespaces}}}
	proxy := &Proxy{
		Labels: labels.Instance{"gateway.networking.k8s.io/gateway-name": "all-namespaces"},
		ServiceTargets: []ServiceTarget{
			{Service: &Service{Hostname: host.Name("same-namespace.gateways.svc.cluster.local"), Attributes: ServiceAttributes{Namespace: namespace}}, Port: ServiceInstancePort{ServicePort: &Port{Port: 80}, TargetPort: 80}},
			{Service: &Service{Hostname: host.Name("all-namespaces.gateways.svc.cluster.local"), Attributes: ServiceAttributes{Namespace: namespace}}, Port: ServiceInstancePort{ServicePort: &Port{Port: 80}, TargetPort: 80}},
		},
	}

	merged := push.mergeGateways(proxy)
	if merged == nil {
		t.Fatal("mergeGateways() returned nil")
	}
	if got, want := len(merged.GatewayNameForServer), 1; got != want {
		t.Fatalf("mergeGateways() selected %d Gateways, want %d", got, want)
	}
	for _, got := range merged.GatewayNameForServer {
		if want := namespace + "/all-namespaces"; got != want {
			t.Fatalf("mergeGateways() selected %q, want %q", got, want)
		}
	}
}

func TestGatewayServicesUsesDestinationIndex(t *testing.T) {
	const gatewayName = "gateways/all-namespaces-istio-autogenerated-k8s-gateway-http"
	backend := &Service{
		Hostname:   host.Name("infra-backend-v3.gateways.svc.cluster.local"),
		Attributes: ServiceAttributes{Namespace: "gateways"},
	}
	...
	push := &PushContext{
		...
		virtualServiceIndex: virtualServiceIndex{
			destinationsByGateway: map[string]sets.String{
				gatewayName: sets.New(backend.Hostname.String()),
			},
		},
	}
	proxy := &Proxy{
		SidecarScope: &SidecarScope{services: []*Service{backend}},
		MergedGateway: &MergedGateway{
			GatewayNameForServer: map[*networking.Server]string{server: gatewayName},
		},
	}

	got := push.GatewayServices(proxy, nil)
	assert.Equal(t, []*Service{backend}, got)
}
